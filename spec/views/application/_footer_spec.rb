...
RSpec.describe 'application/_footer', type: :view do
  ...
  context 'when account is signed in' do
    ...
    it 'has a link to notifications' do
      render
      expect(rendered).to have_link 'Notifications', href: notifications_path
    end

    it 'has a link to profile' do
      render
      expect(rendered).to have_link 'Profile', href: profile_path(account.user)
    end

    it 'has a link to settings' do
      render
      expect(rendered).to have_link 'Settings', href: edit_account_path
    end

    it 'has a link to log out' do
      render
      expect(rendered).to have_link 'Log Out', href: destroy_session_path
    end
